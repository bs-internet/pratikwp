...
class PratikWp_Nav_Menu_Widget extends Widget_Base {
    ...
    protected function render() {
        ...
        $css_classes[] = 'mobile-breakpoint-' . $settings['mobile_breakpoint'];
        ...
        <div <?php echo $this->get_render_attribute_string('wrapper'); ?> <?php echo $schema_attrs; ?>>
            <?php if ($settings['layout'] === 'toggle' || $settings['mobile_breakpoint'] !== 'never') : ?>
                <button class="mobile-menu-toggle d-<?php echo $settings['mobile_breakpoint']; ?>-none" type="button" aria-label="<?php esc_attr_e('Menüyü Aç/Kapat', 'pratikwp'); ?>">
            ...
            <div class="menu-container <?php echo ($settings['layout'] !== 'toggle') ? 'd-none d-' . $settings['mobile_breakpoint'] . '-block' : 'mobile-menu-panel'; ?>">
                <nav class="navbar" <?php echo ($settings['enable_schema'] === 'yes') ? 'itemprop="name"' : ''; ?>>
                    <?php
                    // Menu arguments
                    $menu_args = [
                        'menu' => $settings['menu'],
                    ...
                    if ($settings['submenu_indicator'] !== 'none') {
                        $menu_args['submenu_indicator'] = $settings['submenu_indicator'];
                        $menu_args['custom_indicator'] = $settings['custom_indicator'] ?? '';
                    }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const toggleBtn = document.querySelector('.pratikwp-nav-menu .mobile-menu-toggle');
            const menuPanel = document.querySelector('.pratikwp-nav-menu .mobile-menu-panel');
            const menuOverlay = document.querySelector('.pratikwp-nav-menu .mobile-menu-overlay');
            ...
            if (toggleBtn && menuPanel) {
                toggleBtn.addEventListener('click', function() {
                    ...
                    if (isOpen) {
                        ...
                        toggleBtn.querySelector('.toggle-open').style.display = '';
                        toggleBtn.querySelector('.toggle-close').style.display = 'none';
                    } else {
                        menuPanel.classList.add('active');
                        document.body.classList.add('mobile-menu-open');
                        toggleBtn.querySelector('.toggle-open').style.display = 'none';
                        toggleBtn.querySelector('.toggle-close').style.display = '';
                    }
                });
        ...
        <style>
        .pratikwp-nav-menu .mobile-menu-panel {
            position: fixed;
            top: 0;
            <?php echo (strpos($settings['mobile_menu_position'], 'right') !== false) ? 'right: -100%;' : 'left: -100%;'; ?>
            height: 100vh;
            background: #ffffff;
            z-index: 9999;
        ...
        .pratikwp-nav-menu .mobile-menu-panel.active {
            <?php echo (strpos($settings['mobile_menu_position'], 'right') !== false) ? 'right: 0;' : 'left: 0;'; ?>
        }
        
        .pratikwp-nav-menu .mobile-menu-overlay {
            position: fixed;
            top: 0;
            left: 0;
            width: 100%;
        ...
        body.mobile-menu-open .pratikwp-nav-menu .mobile-menu-overlay {
            opacity: 1;
            visibility: visible;
        }
        
        .pratikwp-nav-menu .mobile-menu-toggle {
            background: none;
            border: none;
            cursor: pointer;
            padding: 0.5rem;
        ...
        <?php if ($settings['submenu_indicator'] !== 'none') : ?>
        .pratikwp-nav-menu .dropdown-toggle::after {
            content: '';
            <?php if ($settings['submenu_indicator'] === 'arrow') : ?>
                border-top: 0.3em solid;
                border-right: 0.3em solid transparent;
                border-left: 0.3em solid transparent;
            <?php elseif ($settings['submenu_indicator'] === 'plus') : ?>
                content: '+';
                border: none;
                font-weight: bold;
            <?php elseif ($settings['submenu_indicator'] === 'caret') : ?>
                content: '▼';
                border: none;
                font-size: 0.8em;
            <?php endif; ?>
